...
object convertMacro {
    def impl(c: Context)(annottees: c.Expr[Any]*): c.Expr[Any] = {
        ...
        def expandTrait(input: BusinessInput):List[Tree] = {
            println(showRaw(q""" def roll(a: cy[bly]): die = a match {
            ...
            input.fixed.map( (fix:FixedPoint) => {
                ...
                """) ++ List(q"""
                    sealed trait $newtrait[R] {
                        def map[S](f: R => S)(implicit sf: Functor[List]): 
                            $newtrait[S]
                    }
                """) ++ List(q"""
                    object $newtraitTerm {
                        implicit def treeFunctor: Functor[$newtrait] = new Functor[$newtrait] {
        ...
        def callSfMapOnLists(x:List[ValDef],t:TypeName):List[Tree] = {println(x);println(showRaw(x));x match {
        ...
        def expandClasses(input: BusinessInput):List[Tree] = {
            ...
            input.variants.map( (vari:Variant) => {
                ...
                val typ      = q"type R"
                val typName  = newTypeName("R")
                List(q"""
                    case class ${vari.name}(..${vari.valParams}) extends ${vari.extend} { 
                        def unroll = $newterm(..${valDefsToValRefs(vari.valParams)}) 
                    }
                """) ++ List(q"""
                    case class ${newTypeName(vari.name.toString+"F")}[$typ](..${valDefsAdaptListType(vari.valParams,oldtrait,typName)}) extends $newtrait[${typName}] {
                        def map[S](f: $typName => S)(implicit sf: Functor[List]): $newtrait[S] = $newterm(..${callSfMapOnLists(vari.valParams,oldtrait)})
                    }
                """)
            }
            ).flatten
